...
class FullScreenQtPreviewBase:
    ...
    def start(self, picam2):
        ...
        if FullScreenQtPreviewBase.thread is None:
            FullScreenQtPreviewBase.previewcreateq = Queue()
            FullScreenQtPreviewBase.thread = threading.Thread(target=self.thread_func, args=(FullScreenQtPreviewBase.previewcreateq,))
            FullScreenQtPreviewBase.thread.setDaemon(True)
            FullScreenQtPreviewBase.thread.start()
            self.event.wait()
        retq = Queue()
        FullScreenQtPreviewBase.previewcreateq.put((Command.CREATE, retq, (self, picam2)))
        self.qpicamera2 = retq.get()

    def stop(self):
        if self.qpicamera2:
            retq = Queue()
            FullScreenQtPreviewBase.previewcreateq.put((Command.DELETE, retq, (self, self.qpicamera2)))
            retq.get()

    def fin(self):
        if FullScreenQtPreviewBase.thread:
            FullScreenQtPreviewBase.previewcreateq.put((Command.FIN, None, None))
            FullScreenQtPreviewBase.thread.join()
            FullScreenQtPreviewBase.thread = None
...
class FullScreenQtPreview(FullScreenQtPreviewBase):
...
class FullScreenQtGlPreview(FullScreenQtPreviewBase):
    def make_picamera2_widget(self, picam2, width=640, height=480, transform=None):
